...
# Sentence splitting
korean_dataset["input_text_long"] = korean_dataset["input_text_long"].apply(
    sent_tokenize
)
...
print()
for corpus in korean_dataset["input_text_long"]:
    if brk % 100 == 0:
...
train_embs, test_embs, train_masks, test_masks, train_labels, test_labels = (
    train_test_split(
        padded_embeddings,
        padding_mask,
        korean_dataset["difficulty_label"].to_numpy(),
        test_size=0.2,
        stratify=korean_dataset["difficulty_label"],
        random_state=42,
    )
)
